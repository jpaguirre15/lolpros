...
def scrape_teams():
    ...
    # Print the entire page or a snippet to debug
    ...
    # Example: Finding team stats
    for row in table.find_all('tr')[1:]:
        cols = row.find_all('td')
        team_name = cols[0].text.strip()
        relative_link = cols[0].find('a')['href']
        team_link = f'{base_url}{relative_link}'
        win_rate = cols[4].text.strip()  # Adjusted column index for win rate
        kda = cols[5].text.strip()       # Adjusted column index for KDA
    
        # Extracting individual columns after KDA
        ...
        towers_killed = cols[11].text.strip()
...
def scrape_and_store_data():
    ...
    with open('lck_teams_stats.json', 'w') as f:
        json.dump(teams, f, indent=4)
    
    print("Data scraped and stored in lck_teams_stats.json")
